...
const REFERENCES = [
  {
    quote:
      'Jason brings a creativity to technology rarely seen in many developers ... Jason continues to be a resource that I count on not only from a development perspective but also as a trusted advisor for future development and the evolution of our technology platforms.',
...
const List = props => {
  ...
  const listItems = items.map((item, index) => (
    <React.Fragment key={index}>
      ...
      <cite>
        - {item.cite_name} ({item.company_name})
      </cite>
    </React.Fragment>
  ))
